...
bot.command('users', async (ctx) => {
  ...
  const keyboard = users.map((el) => Markup.button.callback(el.name, `select#${el.chatId}`));
...
// (select) + (hash sign) + (optional minus) + (user/chat/group id)
bot.action(/^select#(-?[0-9]+)$/, async (ctx) => {
